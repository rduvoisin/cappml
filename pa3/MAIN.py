...
Test.info()
train = Train.copy()
# train, holdout = cross_validation.train_test_split(Train, test_size = 0.10)
# train_for_holdout = holdout.dropna(how = 'any')
...
# np.log1p(train.MonthlyIncome).hist(bins=50)
# plt.figure()
fig, axs = plt.subplots(1,2)
np.log1p(train[train.MonthlyIncome == 0].DebtRatio).hist(bins=50, ax=axs[0])
np.log1p(train.DebtRatio).hist(bins=25)
np.log1p(train[train.MonthlyIncome > 0].DebtRatio).hist(bins=50, ax=axs[1])
mquantiles(train.DebtRatio, np.linspace(0, 1, 10))
np.log1p(train[train.MonthlyIncome == 0].DebtRatio).hist(bins=50, label ='==0', normed=True)
np.log1p(train.DebtRatio).hist(bins=25)
np.log1p(train[train.MonthlyIncome > 0].DebtRatio).hist(bins=50, label ='> 0', normed=True).legend(loc='upper right')
plt.show()      

# Inspect age, has a mysterious minimum of zero.
train.age.hist(bins = np.arange(0, 100, 5))


# Regularizing penalizes model complexity.  LASSO log regression with an L1 penalty|Bi| will force some of your coefficients to zero.
# KNN becomes expensive with a lot of features, so you may want to get rid of some.
...
g = sns.pairplot(data=train[['age','MonthlyIncome','DebtRatio',
                             ...
                             'SeriousDlqin2yrs']].dropna(), hue = 'SeriousDlqin2yrs')
...
train_alldropped, derived_train, train_missing, imputation_candidates = \
    decode_and_drop_missings(train,
